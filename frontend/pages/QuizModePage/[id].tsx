...
const QuizModePage = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch(`http://localhost:5000/getQuestionsByQuizSet/${id}`);
  ...
  const confirmShuffleQuestions = async () => {
    ...
    try {
      ...
      const shuffledResponse = await fetch(`http://localhost:5000/shuffleQuestions/${id}`, { method: 'POST' });
  ...
  const fetchUserSelections = async () => {
    try {
      const response = await fetch(`http://localhost:5000/getUserSelections/${id}`);
      if (!response.ok) throw new Error('Network response was not ok');
  ...
  const fetchEyeIconState = async () => {
    const response = await fetch(`http://localhost:5000/getEyeIconState/${id}`);  // Use quiz set ID
    const data = await response.json();
    if (response.ok) {
  ...
  const fetchFavorites = async () => {
    try {
      const response = await fetch(`http://localhost:5000/getFavorites/${id}`);
      if (!response.ok) throw new Error('Network response was not ok');
  ...
  const handleOptionSelect = async (optionIndex: number | null) => {
    ...
    await fetch('http://localhost:5000/updateUserSelection', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
const updateScore = async (questionId: number, scoreChange: number) => {
  ...
  const updateQuizSetStatus = async (status: string) => {
    try {
      await fetch(`http://localhost:5000/updateQuizSetStatus/${id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const updateScoreInDatabase = (score: number) => {
    ...
    fetch(`http://localhost:5000/updateQuizSetScore/${id}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleReset = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/resetQuestions/${id}`, {
        method: 'POST'
      });
      if (!response.ok) throw new Error('Network response was not ok');
  ...
  const toggleFlipCardVisibility = () => {
    ...
    fetch(`http://localhost:5000/updateEyeIconState/${id}`, {  // Use quiz set ID
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
