...
const DynamicQuizTable = () => {
  ...
  const fetchQuizSets = async () => {
    try {
      const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/getQuizSets`);
      ...
      const updatedQuizSets = await Promise.all(quizSetsData.map(async (quizSet: QuizSet) => {
        const detailsResponse = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/getQuizSetDetails/${quizSet.id}`);
        const details = await detailsResponse.json();
  
        const scoreResponse = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/getQuizSetScore/${quizSet.id}`);
        const scoreData = await scoreResponse.json();
  
        // Fetch lock state
        const lockStateResponse = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/getLockState/${quizSet.id}`);
  ...
  useEffect(() => {
    ...
    const fetchLockState = async () => {
      try {
        const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/getLockState/global`); // Assuming 'global' as a key for global lock state
  ...
  const toggleLockState = async (quizSetId: string) => {
    ...
    try {
      const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/toggleLockState/${quizSetId}`, {
        method: 'POST',
      });
      if (response.ok) {
  ...
  const toggleLock = async () => {
    try {
        const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/toggleLockState/global`, {
            method: 'POST',
        });
        if (response.ok) {
            const data = await response.json();
  ...
  const handleRename = async (quizSetId: string, newTitle: string) => {
    try {
      const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/renameQuizSet/${quizSetId}`, {
        method: 'PUT',
  ...
  const handleDeleteQuizSet = async () => {
    if (deleteQuizSetId) {
      try {
        const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/deleteQuizSet/${deleteQuizSetId}`, {
          method: 'DELETE',
        });
        if (response.ok) {
