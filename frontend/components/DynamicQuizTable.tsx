...
const DynamicQuizTable = () => {
  ...
  const fetchQuizSets = async () => {
    try {
      ...
      const updatedQuizSets = await Promise.all(quizSetsData.map(async (quizSet: QuizSet) => {
        const detailsResponse = await fetch('http://k8s-threetie-mainlb-b5b9250791-1739950720.ap-southeast-2.elb.amazonaws.com/api/getQuizSetDetails/${quizSet.id}');
        const details = await detailsResponse.json();
  
        const scoreResponse = await fetch('http://k8s-threetie-mainlb-b5b9250791-1739950720.ap-southeast-2.elb.amazonaws.com/api/getQuizSetScore/${quizSet.id}');
        const scoreData = await scoreResponse.json();
  
        // Fetch lock state
        const lockStateResponse = await fetch('http://k8s-threetie-mainlb-b5b9250791-1739950720.ap-southeast-2.elb.amazonaws.com/api/getLockState/${quizSet.id}');
  ...
  const toggleLockState = async (quizSetId: string) => {
    ...
    try {
      const response = await fetch('http://k8s-threetie-mainlb-b5b9250791-1739950720.ap-southeast-2.elb.amazonaws.com/api/toggleLockState/${quizSetId}', {
        method: 'POST',
      });
      if (response.ok) {
  ...
  const handleRename = async (quizSetId: string, newTitle: string) => {
    try {
      const response = await fetch('http://k8s-threetie-mainlb-b5b9250791-1739950720.ap-southeast-2.elb.amazonaws.com/api/renameQuizSet/${quizSetId}', {
        method: 'PUT',
  ...
  const handleDeleteQuizSet = async () => {
    if (deleteQuizSetId) {
      try {
        const response = await fetch('http://k8s-threetie-mainlb-b5b9250791-1739950720.ap-southeast-2.elb.amazonaws.com/api/deleteQuizSet/${deleteQuizSetId}', {
          method: 'DELETE',
        });
        if (response.ok) {
