...
const DynamicQuizTable = () => {
  ...
  const fetchQuizSets = async () => {
    try {
      const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/getQuizSets`);
      ...
      const updatedQuizSets = await Promise.all(quizSetsData.map(async (quizSet: QuizSet) => {
        const detailsResponse = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/getQuizSetDetails/${quizSet.id}`);
        const details = await detailsResponse.json();
  
        const scoreResponse = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/getQuizSetScore/${quizSet.id}`);
        const scoreData = await scoreResponse.json();
  
        // Fetch lock state
        const lockStateResponse = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/getLockState/${quizSet.id}`);
  ...
  useEffect(() => {
    ...
    const fetchLockState = async () => {
      try {
        const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/getLockState/global`); // Assuming 'global' as a key for global lock state
  ...
  const toggleLockState = async (quizSetId: string) => {
    ...
    try {
      const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/toggleLockState/${quizSetId}`, {
        method: 'POST',
      });
      if (response.ok) {
  ...
  const toggleLock = async () => {
    try {
        const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/toggleLockState/global`, {
            method: 'POST',
        });
        if (response.ok) {
            const data = await response.json();
  ...
  const handleRename = async (quizSetId: string, newTitle: string) => {
    try {
      const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/renameQuizSet/${quizSetId}`, {
        method: 'PUT',
  ...
  const handleDeleteQuizSet = async () => {
    if (deleteQuizSetId) {
      try {
        const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/deleteQuizSet/${deleteQuizSetId}`, {
          method: 'DELETE',
        });
        if (response.ok) {
