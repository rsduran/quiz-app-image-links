...
const ScrapingSection: React.FC<ScrapingSectionProps> = ({ onScrapeComplete, quizSetTitle }) => {
  ...
  const handleScrape = async () => {
    ...
    try {
      const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/startScraping`, {
        method: 'POST',
...
const downloadQuizPdf = async (quizSetId: string) => {
  const response = await fetch(`http://backend-service.three-tier.svc.cluster.local:5000/api/downloadQuizPdf/${quizSetId}`);
