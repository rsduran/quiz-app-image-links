...
const ScrapingSection: React.FC<ScrapingSectionProps> = ({ onScrapeComplete, quizSetTitle }) => {
  ...
  const handleScrape = async () => {
    ...
    try {
      const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/startScraping`, {
        method: 'POST',
...
const downloadQuizPdf = async (quizSetId: string) => {
  const response = await fetch(`http://k8s-threetie-backends-8cdf5fd76a-49e658ee327e19e3.elb.ap-southeast-2.amazonaws.com/api/downloadQuizPdf/${quizSetId}`);
