...
const AdditionalInfo = ({ url, explanation, discussion_link, question_id, questionDetails }: AdditionalInfoProps) => {
  ...
  const fetchDiscussionComments = async () => {
    try {
      const response = await fetch(`http://localhost:5000/getDiscussionComments/${question_id}`);
  ...
  const handleRocketClick = async () => {
    ...
    if (!fetchedExplanation) {
      try {
        const response = await fetch(`http://localhost:5000/getFurtherExplanation/${question_id}`);
        const data = await response.json();
        if (data.explanation) {
          ...
          const newResponse = await fetch('http://localhost:5000/getFurtherExplanation', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(questionDetails)
          });
          const newData = await newResponse.json();
          await fetch('http://localhost:5000/saveFurtherExplanation', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
