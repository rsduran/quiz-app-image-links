...
const AdditionalInfo = ({ url, explanation, discussion_link, question_id, questionDetails }: AdditionalInfoProps) => {
  ...
  const fetchDiscussionComments = async () => {
    try {
      const response = await fetch(`http://k8s-threetie-backends-53ff77a1f2-63fc387682774c76.elb.ap-southeast-2.amazonaws.com/api/getDiscussionComments/${question_id}`);
  ...
  const handleRocketClick = async () => {
    ...
    if (!fetchedExplanation) {
      try {
        const response = await fetch(`http://k8s-threetie-backends-53ff77a1f2-63fc387682774c76.elb.ap-southeast-2.amazonaws.com/api/getFurtherExplanation/${question_id}`);
        const data = await response.json();
        if (data.explanation) {
          ...
          const newResponse = await fetch(`http://k8s-threetie-backends-53ff77a1f2-63fc387682774c76.elb.ap-southeast-2.amazonaws.com/api/getFurtherExplanation`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(questionDetails)
          });
          const newData = await newResponse.json();
          await fetch(`http://k8s-threetie-backends-53ff77a1f2-63fc387682774c76.elb.ap-southeast-2.amazonaws.com/api/saveFurtherExplanation`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
